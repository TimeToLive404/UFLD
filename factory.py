from utils.loss import SoftmaxFocalLoss, ParsingRelationLoss, ParsingRelationDis, Curloss
...
def get_loss_dict(cfg):
    ...
    if cfg.use_aux:
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'aux_loss', 'relation_dis','curloss'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss(), torch.nn.CrossEntropyLoss(), ParsingRelationDis(),Curloss()],
            'weight': [1.0, cfg.sim_loss_w, 1.0, cfg.shp_loss_w,cfg.cur_loss_w],
            'data_src': [('cls_out', 'cls_label'), ('cls_out',), ('seg_out', 'seg_label'), ('cls_out',),('cls_out',)]
        }
    else:
        loss_dict = {
            'name': ['cls_loss', 'relation_loss', 'relation_dis','curloss'],
            'op': [SoftmaxFocalLoss(2), ParsingRelationLoss(), ParsingRelationDis(),Curloss()],
            'weight': [1.0, cfg.sim_loss_w, cfg.shp_loss_w,1],
            'data_src': [('cls_out', 'cls_label'), ('cls_out',), ('cls_out',),('cls_out',)]
        }
