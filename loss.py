...
class ParsingRelationLoss(nn.Module):
    ...
    def forward(self,logits):
        ...
        for i in range(0,h-1):
            loss_all.append(i/h*logits[:,:,i,:] - i/h*logits[:,:,i+1,:])
        #loss0 : n,c,w
...
class ParsingRelationDis(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(diff_list1)-1):
            loss += self.l1(i/len(diff_list1)*diff_list1[i],i/len(diff_list1)*diff_list1[i+1])
        loss /= len(diff_list1) - 1
        return loss

class Curloss(nn.Module):
    def __init__(self):
        super(Curloss, self).__init__()
        self.l1 = torch.nn.L1Loss()
        # self.l1 = torch.nn.MSELoss()
    def forward(self, x):
        n,dim,num_rows,num_cols = x.shape
        ...
        loss = 0
        for i in range(len(diff_list1)-1):
            loss += self.l1((1-i/len(diff_list1))*0.5*diff_list1[i],(1-i/len(diff_list1))*diff_list1[i+1])
        loss /= len(diff_list1) - 1
        return loss
